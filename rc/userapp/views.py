...
def quiz_post(request):
    ...
    if request.method == 'POST':
        ...
        idk=user_to_update.decision
        
        response_to_update = Response.objects.filter(user=user.id).last()
        present_time = datetime.now()
        present_time = (present_time.hour * 3600) + (present_time.minute * 60) + (present_time.second)
        end_time = (user_to_update.predicted_logout_time.hour * 3600) + (
                    user_to_update.predicted_logout_time.minute * 60) + (user_to_update.predicted_logout_time.second)
        timer = end_time - present_time
        if timer <= 0:
            return redirect('logout')
        ...
        if user_to_update.decision == 0:
            ...
            present_time = (present_time.hour * 3600) + (present_time.minute * 60) + (present_time.second)
            end_time = (user_to_update.predicted_logout_time.hour * 3600) + (
                        user_to_update.predicted_logout_time.minute * 60) + (
            ...
            if int(attempt_1) == Question.objects.get(id=user_to_update.current_ques_id).correct_ans:
                ...
                return render(request,'userapp/quiz.html',{'abc':idk, "remain_time": converted_time, 'question': Question.objects.get(id=user_to_update.current_ques_id),
                ...
                return render(request, 'userapp/quiz.html',
                              {'question': Question.objects.get(id=user_to_update.current_ques_id),
                               'sr': user_to_update.no_question_solved + 1, 'score': user_to_update.current_score,
                               "remain_time": converted_time, 'abc': idk})
        elif idk == 1:
            ...
            return render(request,'userapp/quiz.html',{'abc':idk, 'question': Question.objects.get(id=user_to_update.current_ques_id),
                               'sr': user_to_update.no_question_solved + 1, 'score': user_to_update.current_score,"remain_time": converted_time})
